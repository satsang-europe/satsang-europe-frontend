...
const fetchCongregationsFromStrapi = async (): Promise<PreviousEvents[]> => {
  const res = await strapiApi.get("/congregations?populate=*");
...
const eventQueryOptions = () =>
  queryOptions({
    ...
    queryFn: () => fetchCongregationsFromStrapi(),
  });

export const Route = createFileRoute("/events/previouscongregations/")({
...
function CongregationsPage() {
  ...
  const prevCongregations = events.sort(
  ...
  const totalPages = Math.ceil(prevCongregations.length / eventsPerPage);
  const indexOfLast = currentPage * eventsPerPage;
  const indexOfFirst = indexOfLast - eventsPerPage;
  const currentEvents = prevCongregations.slice(indexOfFirst, indexOfLast);
  return (
    <>
