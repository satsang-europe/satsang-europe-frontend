...
const fetchSeminarsFromStrapi = async (): Promise<PreviousEvents[]> => {
  const res = await strapiApi.get("/seminars?populate=*");
...
const eventQueryOptions = () =>
  queryOptions({
    queryKey: ["seminars"],
    queryFn: () => fetchSeminarsFromStrapi(),
  });

export const Route = createFileRoute("/events/previousseminars/")({
...
function SeminarsPage() {
  ...
  const prevSeminars = events.sort(
  ...
  const totalPages = Math.ceil(prevSeminars.length / eventsPerPage);
  const indexOfLast = currentPage * eventsPerPage;
  const indexOfFirst = indexOfLast - eventsPerPage;
  const currentEvents = prevSeminars.slice(indexOfFirst, indexOfLast);
  return (
    <>
